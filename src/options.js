import { isString, isVisibilitySupported, stringifyArray, VISIBILITIES } from "./utils.js";
...
function validate(options) {
    ...
    if ("ignoredVisibilities" in options) {
        ...
        if (!options.ignoredVisibilities.every(isVisibilitySupported)) {
            const notSupported = options.ignoredVisibilities.filter((iv) => !isVisibilitySupported(iv));
...
const getFeaturesEmptyString = (supported) => `options.features must contain at least one feature. ${
 ...
 */
const getFeaturesNotSupportedString = (notSupported, supported) => `${`Features [${stringifyArray(notSupported)}] in `
