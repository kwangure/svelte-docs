...
import { hasOwnProperty, parseComment } from "./utils.js";
import EventEmitter from "events";
import { Parser as HtmlParser } from "htmlparser2-svelte";
import { parseAndMergeKeywords } from "./v3-utils.js";
import { TemplateEvent } from "./events.js";
        ...
        return {
            ...
            onattribute: (name, value) => {
                if (this.includeSourceLocations
                    && parser.startIndex >= 0
                    && parser.endIndex >= parser.startIndex) {
                ...
                if (this.features.includes("events")) {
                    if (lastTagName !== "slot") {
                        ...
                        if (name.length > 3
                            && name.indexOf("on:") === 0
                            && !value) {
                            const nameWithModificators = name.substr(3)
                                .split("|");
                            ...
                            const baseEvent = {
                                ...
                                locations: this.includeSourceLocations
                                && hasOwnProperty(lastAttributeLocations, name)
                                    ? [lastAttributeLocations[name]]
                                    : null,
                            };
            ...
            onopentag: (tagName, attrs) => {
                const isNotStyleOrScript = !["style", "script"]
                    .includes(tagName);
                ...
                if (tagName === "slot") {
                    if (this.features.includes("slots")) {
                        ...
                        const parsedComment
                            = parseAndConsumeLastComment("public");
                    ...
                    if (this.features.includes("refs")) {
                        if (hasOwnProperty(attrs, "bind:this")
                            && attrs["bind:this"]) {
