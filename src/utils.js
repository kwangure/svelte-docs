...
const getVisibility = (keywords, defaultVisibility) => {
    const keyword = keywords
        .find((keyword) => isVisibilitySupported(keyword.name));
...
const parseComment = (text, defaultVisibility = DEFAULT_VISIBILITY) => {
    ...
    while (index < parsedText.length && index !== -1) {
        ...
        if (index === 0) {
            ({ index: indexDescription } = matches);
        }
...
const getCommentFromSourceCode = (node, sourceCode, {
    ...
    if (node) {
        ...
        if (useLeading && comments.leading && comments.leading.length > 0) {
            const leadingCommentIndex = useFirst
                ? 0
                : comments.leading.length - 1;

            lastComment = comments.leading[leadingCommentIndex].value;
        }

        if (useTrailing && comments.trailing && comments.trailing.length > 0) {
            const trailingCommentIndex = useFirst
                ? 0
                : comments.trailing.length - 1;
...
const isStringLiteral = (node) => node.type === "Literal"
...
const buildObjectFromObjectExpression = (node) => {
    if (node.type !== "ObjectExpression") {
        throw new TypeError(
            "Node must be of type 'ObjectExpression' but is",
            node.type
        );
    }
    ...
    node.properties.forEach((property) => {
        if (property.value.type === "ObjectExpression") {
            obj[property.key.name]
                = buildObjectFromObjectExpression(property.value);
...
function tokensInterval(tokens, range) {
    return tokens.filter((item) => item.range[0] > range[0]
...
const buildCamelCase = (text) => text.split("").reduce((state, char) => {
...
const escapeImportKeyword = (code) => code.replace(/import/g, "importX");

const anyType = { kind: "type", text: "any", type: "any" };
const inferTypeFromVariableDeclaration = (variable) => {
    try {
        const { value } = variable.declarator.init;
